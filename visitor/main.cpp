...
// Конкретные классы фигур class Square: public Shape
...
public:
    ...
    double area() const { return 3.14159265359* m_radius; }
    void accept(Visitor& visitor) override {
        visitor.visit(*this);
    }
    double getM_radius(){
    ...
    void visit(Square& square) override {
        for (int i = 0; i < square.getM_side(); i++){
            for(int j = 0; j < square.getM_side(); j++){
                if (i == 0 || i == square.getM_side() - 1) cout << "0";
                else if (j == 0 || j == square.getM_side() - 1) cout << "0";
                else cout << " ";
            }
            cout << endl;
        }
    ...
    void visit(Rectangle& rectangle) override {
        for (int i = 0; i < rectangle.getM_height(); i++){
            for(int j = 0; j < rectangle.getM_width(); j++){
                if (i == 0 || i == rectangle.getM_height() - 1) cout << "0";
                else if (j == 0 || j == rectangle.getM_width() - 1) cout << "0";
                else cout << " ";
            }
            cout << endl;
        }
...
int main() {
    ...
    cout << "Total perimeter: " << perimeterVisitor.perimeter() << std::endl;
